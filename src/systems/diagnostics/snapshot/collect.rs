//! `SimSnapshot` aggregation from `Particles`/`Grid` state -- split out of
//! `snapshot.rs` (was ~330 of that file's ~524 lines, the actual per-particle/
//! per-cell accumulation logic, not the `SimSnapshot`/`SiSnapshot`/
//! `StepTiming` data definitions that stayed behind in the parent module).
//! Pure computation, no behavior change.
