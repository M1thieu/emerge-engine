// Per-particle force fields + sleep/wake scoring, shared by `force_fields.wgsl`
// (standalone pass, used after the ASFLIP fused G2P) and the fused
// `g2p_update_main` in `particles_update.wgsl` (every other scene), which runs it
// right after the particle update instead of as a separate dispatch.
//
// Host module must declare `Particle`, `StepParams` (dt, vel_limit,
// sleep_threshold) and the `step_params` uniform. Field types: 0=disabled,
// 1=GravityWell, 2=Coulomb, 3=AabbConfinement, 4=RadialConfinement,
// 5=UniformElectric, 6=Buoyancy, 7=LinearDrag, 8=SpatialDragCylinder; params
// layout per type is documented on the `GpuFieldEntry` constructors. All
// positions in grid coordinates. material_mask bit i = material i affected.
// MAX_FORCE_FIELDS / MAX_SLEEP_WAKE_TAGS are pipeline-overridable constants.
...
// Applies this substep's force fields, velocity clamp and sleep/wake scoring to
// `*pp`. Returns false when the particle is left untouched (still sleeping, or
// pinned) -- the caller must then skip its write-back; true means `*pp` holds
// the particle's new state (only `v` and `sleeping` can have changed).
fn apply_force_fields(pp: ptr<function, Particle>) -> bool {
    var p = *pp;
