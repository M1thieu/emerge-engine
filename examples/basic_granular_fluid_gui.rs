...
/// `basic_sand_gui.rs`'s established conventions exactly: LMB/RMB push-pull
...
/// cycles between its three real, distinct presets --
/// `saturated_loam` (soft, yields easily), `consolidated_clay` (stiff,
/// slower creep -- a preset previously found unstable, now fixed via
/// `cundall_damping=1.0`+`apic_blend=0.05`; see `granular_fluid_
/// consolidated_clay_settles_with_cundall_damping` in `tests/
/// physics_correctness.rs`, both enabled here by default so this demo shows
/// the already-fixed settled behavior, not the old oscillation), and
/// `cytoplasmic` (soft biological-matrix regime).
///
...
///
///   cargo run --example basic_granular_fluid_gui --features render
...
fn make_sim() -> Simulation {
    ...
    // Real, already-validated quasi-static settling recipe -- see
    // granular_fluid_consolidated_clay_settles_with_cundall_damping
    // (tests/physics_correctness.rs). Enabled by default so this demo shows
    // the material's real settled behavior, not the pre-fix oscillation.
    ...
    let mut solver = Simulation::new(config, spawn(Vec2::new(16.0, 40.0), MAT_LOAM, 11))
    ...
    let _ = solver.add_body(spawn(Vec2::new(48.0, 40.0), MAT_CLAY, 22));
    solver
}
...
impl State {
    async fn new(window: Arc<Window>) -> Self {
        ...
        println!(
            "basic_granular_fluid_gui: {} particles  |  LMB push  RMB pull  D toggle dig  hold P to pour  R reset  Q quit",
            sim.particles().len()
        );
        Self {
            surface,
